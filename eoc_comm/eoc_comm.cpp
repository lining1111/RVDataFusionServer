    ...
    for(size_t i=0; i<downloade_msg->upgrade_dev.size(); i++)
    {
        if(downloade_msg->upgrade_dev[i].dev_type == EOC_UPGRADE_PARKINGAREA)
        {
            ...
            if(downloade_msg->download_status == DOWNLOAD_FINISHED){
                ret = s106_download_send(client, downloade_msg->upgrade_dev[i].comm_guid.c_str(), 2, 1, 0, "下载完成 MD5 OK");
            }else if(downloade_msg->download_status == DOWNLOAD_FILE_NOT_EXIST){
                ret = s106_download_send(client, downloade_msg->upgrade_dev[i].comm_guid.c_str(), 2, 0, 0, "下载文件不存在");
                ...
                ret = s106_download_send(client, downloade_msg->upgrade_dev[i].comm_guid.c_str(), 2, 0, 0, "本地剩余空间不足");
            }else if(downloade_msg->download_status == DOWNLOAD_MD5_FAILD){
                ret = s106_download_send(client, downloade_msg->upgrade_dev[i].comm_guid.c_str(), 2, 0, 0, "MD5校验失败");
            }
        }
