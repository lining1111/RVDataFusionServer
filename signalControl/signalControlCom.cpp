...
int ComFrame_GBT20999_2017::DataItem::getFromBytes(vector<uint8_t> in) {
    int needLen = 6;
    if (in.size() < needLen) {
...
int ComFrame_GBT20999_2017::DataItem::setToBytes(vector<uint8_t> &out) {
    //数据值索引
    ...
    //属性ID
    out.push_back(this->attributeID);
    //元素ID
...
int ComFrame_GBT20999_2017::FrameAll::getFromBytes(vector<uint8_t> in) {
    int needLen = 17;
    if (in.size() < needLen) {
//        throw std::length_error("ComFrame_GBT20999_2017::FrameAll::getFromByte len less " + to_string(needLen));
    ...
    for (int i = 0; i < this->dataItemNum; i++) {
        DataItem dataItem;
        //先获取数据值长度，然后根据长度分割数组
        uint8_t dataItemLen = in[index + 2] + 2;
...
int ComFrame_GBT20999_2017::FrameAll::setToBytes(vector<uint8_t> &out) {
    uint16_t value16;
    uint32_t value32;
    //开始字节
...
void ComFrame_GBT20999_2017::UpdateFrameLength() {
    //计算长度
    vector<uint8_t> outBytes;
    this->frameAll.setToBytes(outBytes);
    int len = outBytes.size();
    //去掉头尾
    this->frameAll.length = (len - 2);
    this->isUpdateLength = true;
}

void ComFrame_GBT20999_2017::UpdateFrameCRC16() {
    //crc16,不包含头尾
    vector<uint8_t> outBytes;
    this->frameAll.setToBytes(outBytes);
    ...
    this->frameAll.crc16 = crc;
    this->isUpdateCRC = true;
}

int ComFrame_GBT20999_2017::UpdateByteLength(vector<uint8_t> &bytes) {
    int needLen = 17;
...
int ComFrame_GBT20999_2017::UpdateByteCRC16(vector<uint8_t> &bytes) {
    int needLen = 17;
    if (bytes.size() < needLen) {
//        throw std::length_error("ComFrame_GBT20999_2017::UpdateByteLength len less " + to_string(needLen));
        return -1;
...
int ComFrame_GBT20999_2017::getFromBytes(vector<uint8_t> in, bool isCheckCRC) {
    int needLen = 17;
    if (in.size() < needLen) {
//        throw std::length_error("ComFrame_GBT20999_2017::getFromBytes len less " + to_string(needLen));
    ...
    //2.根据通信字节流获取帧内容
    this->frameAll.getFromBytes(bytes);
    if (isCheckCRC) {
        vector<uint8_t> inBytes;
...
int ComFrame_GBT20999_2017::setToBytes(vector<uint8_t> &out) {
    vector<uint8_t> bytes;
    //1.根据具体的帧内容变换成字节流
    this->frameAll.setToBytes(bytes);
    //2.添加转义字符
    uint8_t tf = 0x5c;
    vector<uint8_t> needSet = {0x7e, 0x7d, 0x5c};
